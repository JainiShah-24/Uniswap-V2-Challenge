...
try {
    ...
    Write-Host "⚠ Could not set execution policy. You may need to run: powershell -ExecutionPolicy Bypass -File .\submit.ps1" -ForegroundColor Yellow
}
...
if (-not $origin.StartsWith("https://github.com/")) {
  Write-Host "⚠ Origin does not look like a GitHub https URL; continuing anyway."
}
...
if ($commitMessage) {
  ...
  Say "⏭ Skipping commit - no message provided"
}
...
if ($configureSmartCache -eq "Y" -or $configureSmartCache -eq "y") {
  ...
  if ($LASTEXITCODE -eq 0) {
    ...
    if ($LASTEXITCODE -ne 0) {
      Say "⚠ Global installation failed. Trying local installation..."
...
"@
  ...
  do {
    ...
    if ($smartCacheAddSuccess) {
      ...
      Say "⚠ Smart-cache add command encountered errors"
      ...
      if ($retryCount -lt $maxRetries) {
        ...
        Say "⚠ Maximum retry attempts reached ($maxRetries)"
        break
      }
    }
  ...
  if ($smartCacheCommitMsg) {
    ...
    if (-not $smartCacheAddSuccess) {
      Say "⚠ Smart-cache add command had errors after $retryCount attempts"
      ...
      if ($continuePush -ne "Y" -and $continuePush -ne "y") {
        Say "⏭ Skipping push. You can run smart-cache add manually and push later."
        return
      }
    }
    ...
    Say "⏭ Smart cache configuration created but not committed. You can commit manually later."
  }
} else {
  Say "⏭ Skipping smart cache configuration. You can configure it manually later if needed."
}
...
if ($configureRustCrate -eq "Y" -or $configureRustCrate -eq "y") {
  ...
  if (-not $cargoDir) {
    ...
    Say "⏭ Skipping Rust crate automation"
  } else {
...
"@
      ...
      if ($makeUnique -eq "Y" -or $makeUnique -eq "y") {
        ...
        Write-Host "⏸  Press Enter when you're done making changes..."
        Read-Host
      }
      ...
      if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "⚠ Warning: Could not install wasm32-unknown-unknown target" -ForegroundColor Yellow
      }
      ...
      if ($deploySuccess) {
        ...
        if ($contractAddressMatch) {
          ...
          Say "✅ Contract deployed successfully!"
          Write-Host "⚠ Could not extract contract address from output. Check the deployment log above." -ForegroundColor Yellow
        }
      } else {
        Say "⚠ Contract deployment encountered errors"
        Write-Host "Error code: $LASTEXITCODE" -ForegroundColor Yellow
        ...
        if ($continuePushAfterDeploy -ne "Y" -and $continuePushAfterDeploy -ne "y") {
          Say "⏭ Skipping push due to deployment errors. You can deploy and push manually later."
          return
        }
      }
      ...
      if ($rustCommitMsg) {
        ...
        Say "⏭ Rust changes created but not committed. You can commit manually later."
      }
    }
  }
} else {
  Say "⏭ Skipping Rust crate automation. You can configure it manually later if needed."
}